...
if (isset($_POST['resend_otp'])) {
    ...
    if ($row = mysqli_fetch_assoc($res)) {
        ...
        $ins = mysqli_prepare($conn, "INSERT INTO otp_codes (email, otp, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))");
        mysqli_stmt_bind_param($ins, "ss", $email, $otp);
        mysqli_stmt_execute($ins);

        include_once "mailer.php";
        $sent = sendOTPEmail($email, $row['username'], $otp);
        $success = $sent ? "✅ New OTP sent to your email!" : "⚠️ Could not send email. Check mailer.php config.";
    } else {
        ...
        if ($otp_row) {
            ...
            if ($pdata = mysqli_fetch_assoc($pres)) {
                ...
                if (mysqli_stmt_execute($ins)) {
                    ...
                    $error = "Account creation failed: " . mysqli_error($conn);
                }
            } else {
...
const tick = setInterval(() => {
  ...
  if (seconds <= 0) {
    clearInterval(tick);
    timerEl.parentElement.textContent = 'Code expired. Please resend.';
